...
public class LogApiInfoFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (LogTraceUtils.getLogTraceId() == null) {
            ...
            if (isAsyncDispatch(request)) {
                ...
                try {
                    if (isMultipartFormData(request.getContentType())) {
                        Logger.info(String.format("Request: [%s] uri=%s, payload=multipart/form-data", request.getMethod(), request.getRequestURI()));
    ...
    private void logRequest(RequestWrapper request) throws IOException {
        ...
        if (ObjectUtils.isEmpty(content)) {
            Logger.info(String.format("Request: [%s] uri=%s", request.getMethod(), uri));
        } else {
            String payloadInfo = getPayloadInfo(request.getContentType(), content);
            Logger.info(String.format("Request: [%s] uri=%s, %s", request.getMethod(), uri, payloadInfo));
        }
